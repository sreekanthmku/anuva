...
///
/// `NAME_SLOT` marks where her name sits — in three of the four examples, always
/// vocative. It is resolved against the real name, or removed outright, before the
/// prompt is assembled, so the model never sees a placeholder or a stand-in name
/// it could copy into a reply (HER NAME above allows exactly one name, hers).
///
/// The doctor-visit example stays nameless on purpose: it is the follow-up-turn
/// example, and follow-ups are the turns the prompt tells ANU not to re-open with
/// warmth she has already given.
