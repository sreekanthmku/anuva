...
///
/// `NAME_SLOT` marks where her name sits — in all four examples, always vocative
/// and always early in the reply. It is resolved against the real name, or removed
/// outright, before the prompt is assembled, so the model never sees a placeholder
/// or a stand-in name it could copy into a reply (HER NAME above allows exactly one
/// name, hers).
///
/// All four, including the doctor-visit follow-up, because the model generalises
/// per turn SHAPE rather than across the set — see the v12 note in the header.
