...
/**
 * Crash and performance reporting for the doctor portal.
 *
 ...
 *
 *   1. **Replay is masked to the floor.** This portal renders patient names, their bookings and
 *      what they wrote in an anonymous question. `maskAllText` turns all of it into a grey block
 *      while keeping the layout and the click path, which is what actually reproduces a bug. Never
 *      relax this to `mask: [selectors]` — an allow-list of what to hide is one new screen away
 *      from leaking.
...
export function initSentry(): void {
  ...
  Sentry.init({
    ...
    initialScope: { tags: { build: __BUILD_ID__, beta: String(__BETA_MODE__), app: 'doctor-pwa' } },
  });
}
